...
elseif ($office -eq '2') {
    ...
    $exe = if (Test-Path "$Env:ProgramFiles\OfficeDeploymentTool\setup.exe") {
    ...
    if ($exe) { & $exe /configure https://raw.githubusercontent.com/ShadowElixir/PostInstall/refs/heads/main/files/office.xml }
    if ($oact -eq 'y') {
